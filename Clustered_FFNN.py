...
for l in range(0, 16, cluster_size):
    model = Sequential()
    ...
    for c in range(0, cluster_size):
        mu[:,c*K:(c+1)*K] = np.abs(np.delete(mu_RZF_PF_DNN[:,l+c,:].T, big_values, axis=0))
...
#    y_train[y_train < 0.001] = 0.001
    ...
    # Save models (You may adjust the name based on the chosen model)mu_MR_PF_DNN
    model.save(models_filename + 'Trained_models_for_mu_RZF_PF_CDNN_for_AP' + str(l+1))
